...
class Classifier:
    ...
    def train(self, trainfile: str, dev_filename: str =None, device: torch.device =device):
        """Trains the classifier model on the training set stored in file trainfile"""

        # We load the data and lower the text
        data = pd.read_csv(trainfile, sep='\t', header=None, names=['polarity', 'aspect', 'target', 'position', 'sentence'])
    ...
    def predict(self, datafile: str, device: torch.device=device) -> List[str]:
        """Predicts class labels for the input instances in file 'datafile'
        Returns the list of predicted labels
        """
 
        # We load the test data and lower the text
        data_test = pd.read_csv(datafile, sep = "\t", names = ['polarity', 'aspect', 'target', 'position', 'sentence'])
